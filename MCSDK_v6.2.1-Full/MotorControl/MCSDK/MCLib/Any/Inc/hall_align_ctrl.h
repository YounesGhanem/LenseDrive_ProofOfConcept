...
typedef struct
{
  ...
  HALL_Handle_t* pHALL;                 /* Hall objected used by EAC*/
  uint16_t hRemainingTicks;             /*!< Number of tick events remaining to complete the alignment. */
  bool HallAligned;                      /*!< This flag is true if the hall has been aligned at least once, false if
                                             has not. */
...
/* Function used to execute the hall alignment controller */
bool HAC_Exec(HallAlign_Handle_t *pHandle);

/* It returns true if the hall has been aligned at least one time */
bool HAC_IsAligned(HallAlign_Handle_t *pHandle);

/* It sets a restart after an hall alignment */
void HAC_SetRestartState(HallAlign_Handle_t *pHandle, bool restart);

/* Returns true if a restart after an hall alignment has been requested */
