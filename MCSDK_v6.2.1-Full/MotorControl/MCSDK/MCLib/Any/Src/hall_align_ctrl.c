...
/**
  ...
  */
__weak void HAC_Init(HallAlign_Handle_t *pHandle, SpeednTorqCtrl_Handle_t *pSTC, VirtualSpeedSensor_Handle_t *pVSS,
                     HALL_Handle_t *pHALL)
{
  ...
  else
  {
    ...
    pHandle->pVSS = pVSS;
    pHandle->pHALL = pHALL;
    pHandle->HallAligned = false;
    pHandle->HallRestart = false;
#ifdef NULL_PTR_CHECK_ENC_ALI_CTRL
  }
#endif
}
  ...
  else
  {
    ...
    if (pHandle->hRemainingTicks > 0U)
    {
      ...
      if (0U == pHandle->hRemainingTicks)
      {
        ...
        HALL_SetMecAngle(pHandle->pHALL, pHandle->hElAngle / ((int16_t)pHandle->bElToMecRatio));
        pHandle->HallAligned = true;
        retVal = true;
      }
      else
      {
...
/**
  ...
  */
__weak bool HAC_IsAligned(HallAlign_Handle_t *pHandle) //cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_ENC_ALI_CTRL
  return ((NULL == pHandle) ? false : pHandle->EncAligned);
#else
  return (pHandle->HallAligned);
#endif
}

/**
  * @brief  It sets handler HALLrestart variable according to restart parameter
  * @param  pHandle: handler of the current instance of the HallAlignCtrl component.
  * @param  restart: Equal to true if a restart is programmed else false.
  */
__weak void HAC_SetRestartState(HallAlign_Handle_t *pHandle, bool restart)
{
#ifdef NULL_PTR_CHECK_ENC_ALI_CTRL
  if (NULL  == pHandle)
  {
...
/**
  ...
  */
__weak bool EAC_GetRestartState(HallAlign_Handle_t *pHandle) //cstat !MISRAC2012-Rule-8.13
{
#ifdef NULL_PTR_CHECK_ENC_ALI_CTRL
  return ((NULL == pHandle) ? false : pHandle->EncRestart);
#else
  return (pHandle->HallRestart);
#endif
}
